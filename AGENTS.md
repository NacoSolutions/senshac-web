...
When changing `tina/config.ts`, run the real local Tina dev command before
committing:
...
Start it with the sibling `senshac-content` checkout available, allow Tina to
index and generate `tina/tina-lock.json`, then stop the dev server and review
the lockfile diff. Commit the generated lockfile with the schema change and
run `bun run quality` before opening a PR. The lockfile is generated by
`tinacms dev`; `tinacms build` alone is not a replacement. If a schema is
already verified and only a TinaCloud branch reindex is needed, use a
deliberate whitespace-only lockfile change, then use the TinaCloud branch
**Reindex** action and rerun the GitHub contract workflow.
